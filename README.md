...
| `prompts/4-dev.md` | 在 fresh context 中执行**单个任务**（含 TDD + 提交 + 中途断点恢复 + UI anti-pattern 扫描）| ⚠️ **半独立** | 严格依赖 `TASK.md` 中的 `verify` 字段。但你可以临时手写一份"伪 TASK"丢给它跑 |
...
| `prompts/I-intel-scan.md` | **老项目入场扫描**：检测 AI 上下文文档 + 扫代码生成 / 更新 `CONTEXT.md` | ✅ 完全独立 | brownfield 项目首次使用 flow-kit 必跑；老项目架构偏移后重扫 |
...
6. **artifact-first，不是 phase-gated**：阶段可跳过、可回炉，但 `.md` 不能缺席
